...
- **In-tree target format mutators** (category `format`, sniffer-gated): structural ops for targets that had none.
  - `lz4_chunk_mutate` (`core/mutations/lz4.py`, `lz4_read.c`): LZ4 frame FLG/BD/content-size/block-size/skippable-frame edits with header-check, block and content xxh32 repair. Sniffs the magic at offset 1 — byte 0 is the target's mode selector, kept even (frame path).
  - `rar_chunk_mutate` (`core/mutations/rar.py`, `unrar_read.c`): RAR5 vint and RAR4 fixed-width header fields (type, flags, data/pack size, file fields, extra-area records) set to boundary and overlong values, header CRC recomputed so the edit reaches the parser; plus block dup/drop/swap, header insert, deliberate CRC corruption.
...
- **AFL deterministic sweep arm** (`afl_det`, `--op-afl-det`, `core/mutations/afl_det.py`, T1-1, 2026-09-24): AFL's deterministic stages as an index-addressable map `k -> variant` over flip1/4/8/32, arith8/16/32 (±1..`ARITH_MAX`, LE then BE) and interest8/16/32 (repo `INTERESTING_*`, LE then BE). `_op_afl_det` keeps a per-parent cursor (xxh3 key, LRU 4096), emits the next non-no-op variant per pick, and declines when the sweep is done, so the bandit arbitrates how much of the sweep each seed gets; coverage-new outputs become seeds through normal admission. Wider than the in-loop det stage (flip1/flip8/arith8/interest8 only). ~2 µs per variant at 1 KiB. Off by default; in `--hail-mary`. Not A/B validated.
- **Deterministic stage** (`--deterministic`, `services/operators.py::_deterministic_mutation_stream`): bitflip 1/1, byteflip 8/8, arith 8, interesting 8 (33 mutants/byte), capped at `MAX_DET_MUTATIONS` with per-pass quotas and a byteflip-built effector map. A truncated schedule starts at `_det_start = fuzz_count * (cap // 33) mod len`, so successive runs (e.g. after `--resume`) tile the seed instead of re-walking its prefix.
- **UTF-8 sequence mutator** (`utf8_seq_mutate`, `core/mutations/utf8.py`): the third UTF-8 operator, and the only one that decodes. `utf8_widen` rewrites an ASCII byte as a 2-byte overlong and `utf8_insert` splices a `_FUNNY_UNICODE` blob in at a random offset; neither reads the sequences already in the buffer, so they are only ever collateral (measured on an 87-byte mixed document: `utf8_insert` lands inside an existing sequence on 15.4% of draws, `utf8_widen` produced a +1 length delta on 2000/2000). This one locates a well-formed multi-byte sequence, decodes it, and substitutes a form a decoder must reject for a named reason — one mode per rejection path: `truncate` (sequence ends early), `orphan` (continuation byte with no lead), `widen` (overlong, reaching the retired 5- and 6-byte pre-RFC-3629 forms no other operator can produce), `surrogate` (lone half, or the CESU-8 pair of an astral code point), `boundary` (code point on or past a range check, including `F4 90 80 80` for U+110000), `cont_flip` (continuation byte that is not one, length preserved, so the mid-sequence resync path is reached rather than the truncation one). Validity is decided by a strict `decode()` rather than a reimplementation of the overlong/surrogate/range checks. The sequence is the one covering — or nearest after — the offset the mutation loop already drew, within `SEQ_SEARCH_WINDOW` (512, equal to `_REGION_MIN_LEN`), so `_last_mutation_offset` stays a true description of what was touched; beyond that it declines through `_op_declined` rather than mutating elsewhere under its own name. Growth past `max_len` is a decline too, not a clamp — a clamp would cut the sequence the operator exists to have produced
...
- **Constraint-labelled FSM formats** (`core/format_fsm.py`, `--fsm FILE`, `genseed --fsm FILE`): consumer half of StateLifter (Shi et al., USENIX Sec '23, arXiv:2305.13483), which infers a protocol format as an FSM whose transitions carry byte constraints. A spec names `start`/`final` states and `A -> B : <label>` edges; labels are byte sets (`'a' | 'b'-'z' | 0x00 | any`), literals (`"GET "`) or integers (`u8`/`u16le`/`u16be`/`u32le`/`u32be` `[lo,hi]` with optional `%m=r`). Generation walks from a start and samples each label exactly (integers index the arithmetic progression directly — no rejection); a final state offers "stop" as one more option; past `max_len` it follows the shortest path to a final with no RNG draws, and states that cannot reach a final are never entered. `genseed --fsm` writes seeds (the paper's §6.3 AFLNet use). `fsm_regen` (adaptive band, gated on a loaded FSM) re-walks the input as an NFA sweep over offsets, keeps a random valid prefix and regenerates a valid tail; cost is ~2 ms on a fully valid 4 KiB input, ~0 on junk. Not modelled: induction states (`A_k`) and constraints spanning transitions (length fields).
...
- **Format seed generator** (`--learn-format`, `core/format_seed_generator.py`): every `FORMAT_SEED_EVERY_EXECS` (5000) execs, once the previous batch is drained, `Fuzzer._refill_format_seeds` queues up to `FORMAT_SEED_BUDGET` (32) variants of the last fuzzed seed, one learned field edited per variant (length boundaries, CRC stress fills, replayed sensitive ops; magic/padding untouched). `OperatorEngine.mutate` hands out one per round ahead of the bandit ops, so each takes the normal exec/coverage/save path. Stats appear under "Format Seed Generator" in the report.
...
- **Dictionary Thompson sampling** (`--dict-thompson`, `DirichletPicker`; off by default, on under `--hail-mary`): per round, one `p ~ Dir(1 + wins)` over the dictionary, then the round's token indices `~ Cat(p)` (`RandPool.categorical`, inverse CDF). `wins[token]` counts rounds in which the token was inserted and coverage grew (`fuzz_one` rewards or clears every round). Wins are keyed by token bytes, so dictionary truncation (`dictionary[-keep:]`) does not misattribute them, and persist across `--resume` (`dict_picker` state section). One `p` per round is the point: a fresh `p` per index reduces to plain frequency weighting. Cost: 41 µs/round at 1024 tokens vs 8 µs uniform. **Not shown to help.** fuzzgoat, `-D json.dict`, 3k execs, 8 paired seeds: 2W/5L/1T, median −8 edges (n.s.).
- **Gravity splice donor** (`--splice-donor gravity`, `core/gravity.py`; default `uniform`, on under `--hail-mary`): the six corpus-crossing operators (`splice`, `splice_diff_located`, `splice_common_prefix`, `insert_range_from_other`, `crossover`, `fuse_next`) pick their donor by the trade gravity model `P(j|i) ∝ (1+M_i)^α (1+M_j)^β / (d²+ε²)^(γ/2)`. `M_j` = edges the donor has that the base lacks, `M_i` the converse, `d = 1 − J`; all three from one MinHash Jaccard estimate (`|E_i∩E_j| = J(|E_i|+|E_j|)/(1+J)`, `EdgeTracker.seed_overlaps`, one vectorised compare per pick). A donor with `M_j = 0` weighs 0, so near-clones never win; ε=0.05 (Plummer softening) keeps `d=0` finite. Cost: k=8 sampled candidates per pick, O(k·64), not O(N). Exponents start at (1,1,1) and refit every 64 rounds by Poisson PPML (IRLS, ridge 1e-3, clamp ±4) on a 2048-row ring of `(features, new edges)`, zero-yield rounds included; `fuzz_one` credits each round after `record_edges`. Refits need ≥32 positive rows: on fuzzgoat 6 hits in 512 rows drove γ to the clamp (quasi-separation). Exit summary prints α, β, γ, hits and refits; γ ≈ 0 means distance carries no signal. State persists across `--resume` (`gravity` section). **Not shown to help.** fuzzgoat (`fuzzgoat_read_nosan`, 5k execs, 5 paired seeds): 2W/3L, median −4 edges (n.s.; 276/262/223/203/188 vs uniform 210/238/262/232/192). Fit stayed at prior exponents (≤23 hits per 2048 rows, gate closed). Mean eps 58.4 vs 61.0 (−4%, two runs in parallel, noisy). Pick cost 46 → ~34 µs after batching the MinHash compare; uniform path unchanged (one `rng.choice`). `container_signal` (png + jpeg `.so`, 10k execs, 20 paired seeds, `tools/lib/bench_paired.py` arm `splice-gravity`): 20W/19L/1T, McNemar p=1, median +0.5 edges; png 10W/9L/1T (+3), jpeg 10W/10L (−2). Null on all three targets; keep `uniform` default.
...
- **EcoFuzz seed selection** (`--ecofuzz`, port-backlog D2): where Boltzmann weights seeds by pure pick-count rarity blind to whether a seed ever produced anything, EcoFuzz weighs a Laplace-smoothed new-edge reward probability against observed cost — `energy(seed) = reward_prob / cost`, `reward_prob = (coverage_edges + 1) / (fuzz_count + 2)`, `cost = effective_fuzz_count(meta, mean_exec)` (the same cost-ledger normalization Boltzmann's rarity term already relies on). A seed that keeps failing to turn fuzzing into new edges decays toward the trickle rate as its `fuzz_count` grows; a cheap-but-productive seed keeps its energy up regardless of how rarely it's picked. Also an Elo-eligible strategy (`_pick_ecofuzz_seed()` in `seed_picker.py`), and the direct (non-Elo) `pick_seed()` dispatch prefers `boltzmann` over `ecofuzz` when both flags are set.
- **Metropolis corpus admission** (`--metropolis`, requires `--anneal-budget > 0`): swaps the hard `is_new_coverage()` boolean gate for probabilistic acceptance — non-improving inputs are admitted with `P = min(1, exp(-ΔE/T))`, `ΔE = |P| / |M ∪ P|` (`core/metropolis.py`, 2026-09-24): the share of mutant ∪ parent path the parent already explains. Identical, subset (early-exit) and untracked-parent mutants get `ΔE = 1`, the legacy `exp(-1/T)` floor (37% at T=1.0, ~0 at T=0.1); large reroutes through known code are admitted more often. Replaced the constant `ΔE = 1.0`, which made acceptance a clock blind to the candidate. Implemented in `fuzz_one()` (fuzzer.py) after the existing coverage/crash gate.
...
- **Seed baseline calibration** (`Fuzzer._calibrate_seed_baselines`, 2026-08-24): `fuzz_one()` runs ONLY mutated input — `_dedup_mutate()` transforms even seed iterations — so before this pass the tracker's edge universe was built exclusively from mutants that happened to stay format-valid. Structured-target campaigns started blind: observed on `png_read.so`, 11 valid PNGs produced `shm=6` edges after 150 execs (mutants fail validation at the signature check → wrapper-only coverage) while ONE direct execution of a seed records ~330; Good-Turing then read the starved universe as 100% saturated (`sat:` pinned, N2<10 damping), growth projections flatlined, and cmplog token collection saw nothing. The pass executes every loaded corpus seed verbatim once through the normal pipeline (`_run_target` → `is_new_coverage_with_edges` → `EdgeTracker.record_edges`) at startup, before the loop; crashes/timeouts during calibration are counted, not fatal. Runs in single-target SHM mode only; `--no-calibration` skips it (startup time and RSS on wide targets — ffmpeg, 480 seeds: 57 s, +336 MB — at the cost of the blind start above). Measured: 96 real PNGs → 893 baseline edges in 0.7 s, and the campaign's first stats line starts from real signal instead of wrapper noise. Regression tests: `tests/test_regression_seed_baseline.py` (scenarios run in fresh interpreters — dlopen caches the target .so, so a second in-process Fuzzer would reuse a shim mapping pointing at a dead segment).
- **SMT cmplog cache — true LRU with adaptive growth** (`core/smt_solver.py::Z3Solver`, 2026-09-12): `solve_cmplog_pair`'s result cache was documented as LRU but evicted in strict insertion order — a hit never moved its key, so a repeatedly-solved pair could still be pushed out by unrelated inserts while the cache churned. Now an `OrderedDict` moves a key to the end on every hit and on insert, so eviction (`popitem(last=False)`) always removes the true least-recently-used entry. Capacity also adapts: evicted keys are remembered in a bounded "ghost" list (key only, no value), and a miss that lands on a ghost entry counts as a thrashing signal — after `_GHOST_HIT_GROW_THRESHOLD` (8) such signals, `_cache_maxsize` doubles, up to `_CACHE_MAXSIZE_CAP` (16384), so targets with a larger distinct-comparison working set grow past the 1024 default instead of thrashing indefinitely. Regression test: `tests/test_smt_solver.py::TestCaching::test_regression_lru_hit_protects_from_eviction` (fails against the old FIFO order, passes against the fix); adaptive growth covered by falsification (`test_falsification_single_ghost_hit_does_not_grow`) and adversarial (`test_adversarial_grow_capped_at_hard_limit`) tests.
- **CFG decode cache** (`core/cfg_cache.py`, 2026-08-24): `TargetDistance._build_cfgs` decodes through an on-disk cache — one gzip pickle per (binary, decoder) pair under `~/.cache/fuzzer_cfgcache/` (`XDG_CACHE_HOME` aware). Invalidation key folds three components: `NT_GNU_BUILD_ID` (new `elf.build_id()` PT_NOTE parser; falls back to sha256 of file bytes when absent), a decoder fingerprint (sha256 over `cfg.py` + `elf.py` source bytes), and `cfg_cache.SCHEMA_VERSION` plus file size. Artifacts accumulate per function, so later runs with different `--target-functions` still hit. Loads go through a `_SafeUnpickler`-style allowlist (state_store's globals + exactly `FunctionCFG`/`BasicBlock`); corruption or version mismatch logs one warning and recomputes, never raising past the caller. Misses decode in parallel via `ProcessPoolExecutor` (fork) when total bytes ≥ 512 KiB or ≥ 16 functions; below that the serial path is unchanged. Opt out with `--no-cfg-cache` or `FUZZER_DISABLE_CFG_CACHE=1`. Residual caveat: reproducible-toolchain rebuilds can reuse a build-id (narrowed by size in the digest). K-Scheduler W1's whole-program ICFG reuses the same identity/load/store API.
...
- **ICFG centrality** (`core/centrality.py`): `InterproceduralCFG.centrality_scores()` (Brandes betweenness) and `closeness_scores()` (directed out-closeness, Wasserman-Faust corrected so partial reach is penalized). Both share one per-source BFS (`_bfs`). Library-level; no scheduler consumes them yet.
...
### Coverage & Scoring
- **Decohered periodic cadences** (`core/cadence.py`): nineteen subsystems do periodic work keyed on an execution counter — CRPS at 8, Monte-Carlo draw refresh at 16, state snapshots at 20, crash-eta/Katz/Markov at 50, five separate things at 100, GARCH refit at 128, the seed-picker weight recompute at 200, corpus history at 500, three at 1000 and two at 2000. Measured over 20,000 executions: of the ten distinct periods, **zero of the forty-five pairs are coprime**, and the co-firing distribution is bimodal — nothing between four and nine subsystems firing together, then spikes of eighteen and nineteen. A weight recompute alone is ~11.6 ms against ~0.8 ms for an ffmpeg execution, so those iterations cost order 100x a normal one. The periods are not the defect and this module does not touch them: each rate was chosen for a reason, and retuning nineteen of them to be pairwise coprime would change nineteen behaviours to fix a scheduling artifact. What is wrong is that they all measure their period from the same origin. `phase_of(site, period)` derives a fixed per-site offset from CRC-32 of the call-site name — not `hash()`, which is salted per interpreter run and would make a resumed campaign fire on a different schedule than the one it saved — and `due()` / `bucket()` are drop-ins for `counter % period == 0` and `counter // period`. **The rate is identical; only the firing instants move**, and total firings are conserved exactly (asserted). `bucket` advances on exactly the counters where `due` is true, so a cache and the recompute that fills it can be gated on either. **Partially wired, and the numbers below are for what is actually wired, not the ceiling:** seven sites carry offsets (`seed_picker.weights` at 200, `seed_picker.classify` / `seed_picker.pareto` / `fuzzer.dict_eps` / `fuzzer.rss_eps` at 100, `seed_picker.corpus_perplexity` at 500, `crash_eta.invalidate` at 50), which takes max co-firing from **20 to 13** and eliminates the k>=14 tail, leaving a k=12/13 residue at multiples of 2000 from the unwired sites. Offsetting all nineteen would give 19 -> 3. The rest are deliberately untouched: `stats.report` is user-visible and should stay on round numbers, `filesystem.SNAPSHOT_INTERVAL` decides which generations are full snapshots and so carries delta-chain and resume semantics, and `markov`/`katz_channel`/`monte_carlo` gate on self-resetting counters rather than a modulo on a global one, so moving them is a behaviour change rather than a phase shift.
...
- **n-gram edge coverage** (`__AFL_NGRAM_K`): compile-time history depth k — k=2 is a byte-identical compatibility branch keeping the exported `__afl_prev_loc` and XOR ids (corpora/resume stay valid); k>2 swaps in a FIFO ring (`uint32_t` index, `#error`-guarded 2..4096) hashed with FNV-1a, order-sensitive where XOR chains go commutative. Marker symbol `__afl_ngram_k_N` always emitted; `detect_ngram_k()` mirrors `detect_ctx_bits`'s max-across-TUs policy and returns 2 on absence. Map sizing multiplies `ngram_inflation_factor(k)` = min((k−1)², 32). ptrace twin uses a deque XOR chain with `reset_edge_map()` clearing it. Build flavors: `tools/build_targets.sh --ngram` → `png_read_ng2.so`/`png_read_ng3.so` with vendored libpng+zlib rebuilt at trace-pc (whole-program CFG)
- **K-Scheduler node channel + Katz arm** (She et al., S&P'22 port): trace-pc builds upload an ICFG-derived `node_idx` column inside widened 16-byte distance-table entries plus a separate eager-write node bitmap segment (`__AFL_NODE_BITMAP_ID`) — Python reads-and-clears after every exec (no destructor writer, unlike the lazy distance tail). `core/icfg.py` decodes the whole program (0.27 s / 2099 nodes on png_read_ng3), `core/horizon.py` contracts visited nodes with connectivity-preserving shortcuts and Tarjan-SCC DAG conversion, `core/schedulers/seed_katz.py` solves out-degree Katz via one `np.bincount` SpMV per round with βᵢ = 1 − Rᵢ/T from bitmap hit counts. The Elo-arbitrated `katz` seed arm picks centrality-weighted seeds; a `"katz"` power schedule converts normalized centrality into clamped energy. Resume states carry a `coverage_contract` (`ngram_k`, `node_channel`) and refuse mismatches. **Read a Katz campaign's numbers with the paper's own calibration in hand:** the headline gain over AFL-based schedulers is 4.21% arithmetic mean but **1.91% median** at 24h over 10 runs (their Table V), which is inside the noise band of any single-run comparison; the ablations are each larger than the headline — loop removal +21.70% (XII), non-uniform β +24.19% (X), visited-node deletion +24.13% (XIII) — so most of the value is in the graph transform rather than in Katz itself, and none of `horizon.py`'s three steps is optional polish; and their own accuracy check against an ideal scheduler is a Kendall tau of 0.01–0.09, *negative* on libjpeg and openthread (XIX). That is why this is one Elo-arbitrated arm among several and not a replacement for the existing pickers. Note also that `__AFL_CTX_SENSITIVE` qualifies edges by caller context while K-Scheduler's nodes are context-free, so V saturates faster than the edge map and the horizon shrinks earlier than the paper's numbers assume
- **Tang low-rank seed arm** (`core/schedulers/seed_tang.py`, arXiv:1807.04271, 2026-09-07): port of Tang's quantum-inspired recommendation algorithm over the seed x edge matrix (`EdgeTracker.seed_edges` + `seed_hit_counts`), exposed as the Elo-arbitrated `tang` seed arm behind `--tang` (`--tang-rank`, `--tang-refit-interval`). A seed scores by the l2 mass its rank-k coverage row places on the edges it already covers; the module also carries faithful ports of Proposition 4.2 (inner-product estimation from l2 samples), Proposition 4.3 (rejection sampling from `Vw`) and `modfkv_sample_complexity` (Algorithm 2's `q`). Refit is a dense truncated SVD on an interval gate, not ModFKV: at our dimensions ModFKV wants 1e5-1e13 sampled rows against corpora of 13-116 seeds, so the subsample exceeds the input. **Off by default and it should stay that way until an A/B says otherwise.** Measured over ten campaigns: the score predicts, but controlling for `A.sum(1)` (total hit volume, one row sum) it adds nothing — mean partial Spearman +0.006, Wilcoxon p=1.0 — and an l2 draw lands on edges owned by 1.3-1.9x more seeds than a uniform draw, which fights `RARE_EDGE_OWNERS` and the crowding penalty in `seed_picker`. Inverting the score is not a separate option: `frontier_mass = 1 - covered_mass` exactly. Two findings transfer beyond this arm: Tang's low-rank and `(gamma, zeta)`-typicality assumptions are **mutually exclusive** on coverage data (hit counts give rho@10 = 0.05 but drop typicality from 91% to 6-17%; binary reverses both), and reconstruction fidelity is nonetheless real (1.6-6.3x over a popularity baseline under MNAR masking). Full findings, seven methodology gaps, and the rejected variants in `docs/handover/handover_done_2026-09-06.md` §14.
- **LST revisit bound** (`--lst-revisit SECONDS`, P3-3 step 6, 2026-09-24): `Fuzzer._pick_seed` stamps `seed_meta["last_picked"]` (session-local, not persisted). `SeedPicker._pick_lst_seed` treats each seed as a job due `max(last_picked | added_at, start_time) + D` with processing time `mean exec time x mutations_per_input`, and pre-empts every seed strategy with the least-slack seed (`core/job_scheduling.least_slack`) once any slack is negative. A clean scan caches `now + min_slack` and skips the O(n) scan until then (0.2 µs gated, ~0.8 ms at 2000 seeds when overdue). Strategy label `lst` is not an Elo arm. Off by default, not in `--hail-mary`, **not A/B validated**.
- **Entropy leave-one-out seed arm** (`--entropy-loo`, `core/schedulers/seed_entropy_loo.py`, entropy §5 first step, 2026-09-24): weights seeds by `Δ(s) = H(pool) − H(pool \ s)` over pooled corpus bytes, all seeds in one numpy pass over a per-seed count slab (fold on admission, tail-fill unfold on eviction). Weight `max(Δ, 0) + 1e-6` since a flat seed has `Δ < 0`. Known limitation, pinned by test: same-histogram pairs each score 0 where Shapley splits the entropy — full Shapley only if that matters. Rescore ~5 ms at 2000 seeds after a corpus change, 0.5 ms otherwise. Elo arm and non-Elo fallback; in `--hail-mary`. Not A/B validated.
- **Strata arms** (`--strata`, `--op-strata`, `core/edge_ledger.py`, `core/schedulers/seed_strata.py`, `op_strata.py`, 2026-09-24): `EdgeLedger` folds each `record_edges` call's ids into families (`id >> ctx_bits`, 8 on ctx-free builds); frontier = families owned by < 50% of seeds; a family reads at family resolution when the edge-id probe says ids move or its tag occupancy reaches 0.95. Seed arm: Thompson over frontier families, seed within by mean `log1p(N/owner)`; hit = the picked seed's round confirmed a new edge in that family. Op arm: Thompson over `(op, family)` cells with prior `Beta(K·p_op + s, K·(1−p_op) + f)`, K=8; stratum = the strata pick's family, else the parent's rarest family. Both abstain unless the sancov guard is "present". Measured on fuzzgoat (2000 execs, `--strata` sole picker): **0 hits in 1939 picks** — new edges land in new families, never in a known one, so the family posterior only accumulates misses. Speed: 57.5 vs 57.7 eps. Paired bench: `bench_paired.py run --arms strata-a0,strata-a0-ctl,strata-a1,strata-a2,strata-a1-elo,strata-a3,strata-a4` then `bench_paired.py strata results/paired/*_strata-*.json` (control first, A1 non-inferiority and ≥ 0.98 eps ratio, Holm over A2–A4; A3/A4 pair against `strata-a1-elo`). Not A/B validated.
- **Kruskal-count seed arm** (`core/schedulers/seed_kruskal_count.py`, `--kruskal-count`, 2026-09-16): a seed is read as a jump table — byte `b` at offset `p` jumps to `(p + max(1, b)) % n`, or to the next boundary-marker offset when the target profile has a `format_signature`. Four walkers start at evenly spaced offsets and step 256 times; `score = coupled_pairs/6 * (1 - mean_coupling_step/256)`. The jump depends on position only, so coupled walkers stay coupled. The arm draws an anchor by `score + 1e-6`, then copies a score-weighted donor's bytes (modulo donor length) into the anchor along the trajectory the earliest-coupled pair shares after meeting, keeping a detected magic prefix; no coupling returns the anchor unchanged. Elo-eligible whenever the corpus is non-empty; in the non-Elo `pick_seed()` it runs after QEA/GA and before Bayesian. Scores are memoized by content and evicted with the corpus; the byte-fallback path is batch-vectorized (12.6x over the scalar `trace`, which stays as the reference). State (`kruskal_count` section) is five primitive counters, validated whole on load and restored only under `--resume`; report and live stats show scored/coupled/generated/mean. **Off by default, not A/B validated.** Two measured caveats: coupling is strongly length-biased (random bytes: 126/200 seeds couple at 64 B, 67/200 at 512 B, 10/200 at 4 KiB), and boundary markers collapse walkers that start before the first marker onto it at step 1 (synthetic text corpus: 47 distinct scores without markers, 26 with). Plan: `docs/handover/kruskal_count_seed_strategy.md`.
- **Byte-entropy seed arms** (`core/schedulers/seed_entropy_kl.py`, `seed_entropy_zscore.py`, 2026-09-19): two Elo-arbitrated seed arms built on the byte-content entropy signals (`core/byte_entropy.py`), implementing §2 and §3 of `docs/handover/handover_entropy_seed_schedulers_2026-09-19.md`. Both decide *which* seed is picked; neither touches `SeedScorer`, which scales energy once a seed already is.
  - `--entropy-kl` scores a seed by `KL(P_seed || P_corpus)` over the 256 byte values, evaluated in cross-entropy form (`KL = -H_s + P_s · (-log2 Q)`) so the whole corpus is one matvec against a `(-log2 Q)` vector. Sharper than a scalar entropy gap by construction: two seeds uniform over different byte pairs carry identical entropy and opposite divergence. The pooled `Q` is owned by the arm, not read from `Fuzzer._corpus_entropy` (the `byte-ent:` readout, which `CorpusManager` now folds on admission/trim and unfolds on prune). This one folds on admission and unfolds on eviction, so `Q` is exactly the live corpus. Distributions live in one amortized-growth slab that fills evictions from the tail: on a 2000-seed corpus a pick that changes nothing is 0.33 ms, the pick after an admission 1.0 ms (6.6 ms when the slab was re-stacked per change), and each evicted seed unfolds at ~50 µs. `Q` carries `POOL_SMOOTHING = 1/256` pseudo-counts per bin — one pseudo-byte in total, not Laplace's 256, which would dominate any pool under a few KiB; every scored seed is folded before it is scored, so the floor only guards the margin rather than tuning anything.
  - `--entropy-zscore` (`--entropy-zscore-target`) weights a seed by `exp(-((z - target_z)/width)²/2)` on `z = (byte_entropy_pct - mean)/stddev` from a windowed `RunningMoments` (512). This is the fixed-threshold fix: `SeedScorer`'s `ENTROPY_SPARSE_PCT=25` / `ENTROPY_STRUCTURED_PCT=62` / `ENTROPY_RANDOM_PCT=93` are target-agnostic, so on an image/audio/compressed-container target every well-formed seed sits above 93% and the whole corpus lands in one bucket; a z-score against the corpus's own spread self-calibrates with no constant to tune. `target_z` is the only knob and it points the arm (0 = typical for this corpus, positive = the high-entropy tail). Warm-up gate is 20 distinct seeds, the same value `CriticalSlowingDown` uses before trusting its variance signal, **plus a spread floor**: `stddev > 0` is not usable, because `byte_entropy_bits` of a single-symbol input lands at 5.6e-15 rather than 0, so a corpus of constant seeds shows a 1.5e-15 stddev and z-scores that are rounding noise amplified to ±3 (`MIN_SPREAD_PCT = 1e-6`). Elo lists the arm while it is still cold — scoring is what warms it, and it observes the whole corpus inside its first selection — then only while `ready`, so a corpus with no entropy spread stops being offered instead of declining forever. The moments are deliberately **not** persisted: a resume reloads the corpus whole and the first scoring pass rebuilds the calibration from the survivors, where a restored window would count every one of them twice.
  - Substrate: `byte_histogram()` / `entropy_bits_from_counts()` split out of `byte_entropy_bits` (one scan yields both a seed's entropy and its distribution), plus `CumulativeByteEntropy.remove()` and `freq_dist()`. Folding a 4 KiB seed went 167 µs → 24 µs (one `.tolist()` instead of 256 numpy-scalar unboxings, and the clamp confined to the unfold path), which also speeds `load_corpus()`. **Both arms off by default, neither A/B validated.**

### Distribution Diagnostics
- **Pool drift** (`--pool-drift`, `core/pool_drift.py`, 2026-09-25; off by default, on under `--hail-mary`): live corpus byte distribution vs the seed set, frozen in `init_seed_metadata` (after transforms; on `--resume`, the corpus at resume — nothing records original seeds). Lazy set-diff sync on each read, as the KL arm does. Reports `dH = H(pool) − H(seeds)` (direction: noise vs concentration), `JS(pool‖seeds)` in [0, 1] bits (distance; KL is infinite on any new byte value), novel mass (pool bytes on values the seeds lack; what JS dilutes) and `I(seed; byte) = H(pool) − Σ nᵢ/N·H(seedᵢ)` (inter-seed heterogeneity). Status: `drift: dH= js=`; report: Entropy & Diversity. Order-0, first 4 KiB per input. Diagnostic only. Smoke run on fuzzgoat (3 JSON seeds, 233 s): corpus 4 → 168, `JS` 0.008 → 0.608, eps unchanged. Cost: ~20 ms startup per 1000 2 KiB seeds, ~0.6 ms per status tick at 5000 seeds.
...
- **Format learner Fenwick coverage index**: each `FormatCluster` indexes its field hypotheses in a range-add/point-query Fenwick tree over byte offsets (`_CoverageFenwick`): each field is one edge, index 0 is the null-frame root, `depth(pos)` = fields covering `pos`. Zero depth skips the O(H) span scan in `_update_hypotheses`, `_track_values`, `record_liveness` and backtest `_predict_effect`; hits still scan so first-in-list-order wins. Self-heals when `hypotheses` is appended to or reassigned externally; disabled once span mass exceeds the covered extent (dense frame, filter never rejects). Measured (5k records, H≈800-1000): `record()` 50-64 -> 26-29 µs, backtest replay 2.2-3.9x faster; 256-byte dense frame unchanged.
- **Format learner flat `record()` cost**: MAD is read off a `{delta: count}` histogram (`_delta_counts`, exact upper-median semantics, O(distinct deltas)) instead of sorting the unbounded delta history twice per record; only the field a record touches is stride-boosted and re-typed (`_boost_stride` + `_classify`; `_classify_fields` stays as the full pass); `record_liveness` re-types without the boost; the timeline trims in place instead of copying `max_timeline` entries. Stride boost now fires only on evidence for that field — before, every record boosted every aligned field. Measured, 5% effective mutations: per-record 0.1 -> 0.9 ms by 15k records before, 17 -> 32 µs flat-ish over 40k after.
...
- **ASAN releases freed memory**: the ASAN defaults the fuzzer sets (`cli/ldpreload_wrapper.py`, `Fuzzer._setup_asan_env`) include `allocator_release_to_os_interval_ms=1000` unless `ASAN_OPTIONS` already sets it. In-process, ASAN's allocator also serves Python; by default it never returns freed pages, which pinned ~870 MB freed by the Katz ICFG build on ffmpeg. 0 ms costs ~3% exec/s; 1000 ms nothing measurable.
- **Katz ICFG footprint**: `KatzChannel.build` releases the per-function CFGs after the probe table and `upload()` drops `TargetDistance` (it holds the whole binary); `node_addrs` is a sorted `array('Q')` looked up by bisect. ffmpeg (2.74M nodes): 2112 -> 217 MB retained. The recompute (`core/horizon.py::build_horizon_graph`, `seed_katz._dag_depth`) runs on CSR arrays: shortcut walks start only at U nodes with a V successor, both-way peeling leaves just the nodes that can sit on a cycle for an array-backed Tarjan, and DAG depth is level-synchronous Kahn. One recompute on 2.67M nodes: 34.8 s / 1923 MB -> 12.4 s / 300 MB.
...
- **Caching**: the battery costs ~1 ms per 4 KiB window, so profiles are cached per seed by content hash in `OperatorEngine._region_cache` (LRU-bounded at 64 entries via `core/lru.py`; evicting a seed drops its region-liveness entry too). Off by default: worth paying only on structured targets.
...
- **Guidance-source None semantics**: `CrashMITracker.weighted_position` and `MutualInformationTracker.weighted_position` now return `None` instead of `0` when they have no valid data. `OperatorEngine.select_position()` skips `None` candidates and falls back to random selection; this prevents empty-tracking-tracker states from injecting a hardcoded offset-0 candidate that biased region-liveness attribution toward region 0.
...
### GF(2) / GF(2^n) Field Arithmetic
- **Shared polynomial layer** (`core/gf2_common.py`): single source of truth for GF(2) polynomial operations (`poly_mul`, `poly_divmod`, `poly_mod`, `poly_gcd`, `poly_powmod`), Rabin irreducibility testing (`is_irreducible`, `find_irreducible`), the `GF2n` field class (`add`/`mul`/`pow`/`inv`, primitive element search, `omega_powers()`), and a generic `find_primitive_root(order, is_primitive, rng)` helper that accepts any RNG with `.randrange(a, b)` (duck-typed for `random.Random` and `RandPool`). `core/berlekamp_massey.py` imports `poly_gcd` from here — the duplicate `poly_gcd` / `_poly_mod` definitions were removed in favour of the shared implementation. `GF2n.mul`/`pow`/`inv` route through Zech (log/antilog) tables built once from the same omega-cycle enumeration `omega_powers()` already needs, rather than doing a carryless multiply + polynomial long division (or repeated-squaring exponentiation) on every call — this is the standard log-table technique for GF(2^q) arithmetic (Reed-Solomon codecs use the same trick), applied here because the enumeration Cook-Mertz already required for MLE point indexing turned out to be exactly the table the field's own arithmetic needed. Measured (`q=8`, the only size any current caller uses): `mul` ~10x faster, `inv` ~200x faster, verified exhaustively against the retained polynomial-path methods (`_mul_poly`/`_pow_poly`) over all 65536 pairs (`mul`) and all 255 nonzero elements (`inv`, plus the `a*inv(a)==1` field axiom) — see `test_gf2n_log_tables.py`. Table construction is skipped above `_MAX_TABLE_ORDER` (2^20), falling back to the polynomial path transparently, so an unusually large `q` degrades gracefully rather than allocating an enormous table.
- **Cook-Mertz field layer** (`core/cook_mertz.py`): `CookMertzField` wraps `GF2n` for the multilinear extension (MLE) interpolation identity `f(x) = sum_{omega in F*} lambda_omega * f(omega)`. Exposes `powers()` (lazy omega power table), `interpolate(evaluations)` (full Lagrange interpolation over all `omega^i`), and `evaluate_mle(coefficients, point)` (direct MLE evaluation at a single field element). The efficiency claim — `evaluate_mle`'s polynomial has degree <= q in the field variable (each monomial's coefficient contributes `point**popcount(e_idx)`, so degree ranges over `[0, q]`), meaning `q+1` points determine the whole `2**q-1`-entry table via `interpolate()`, not all of them — is verified directly in `test_cook_mertz.py` with both a positive case (forced exact degree `q`, reconstructs from `q+1` points) and a forced negative control (`q` points, one short, provably diverges). `interpolate()` itself benefits from the `GF2n` log-table optimization above (its inner loop is what made the case for building it): measured ~85ms -> ~5.5ms per call at `q=8`, still exact against `evaluate_mle`. **Deliberately left unwired** after checking: none of the fuzzer's existing recursive tree evaluators (grammar generation depth ~10, MCTS depth ~64, alpha-beta search depth ~8-10 in `cond_stmt.py`/`smt_solver.py`) are anywhere near the regime where trading time for space pays off — their per-node values are a few bytes and their whole call stack costs kilobytes, while even the optimized `interpolate()` at `q=8` is still ~4 orders of magnitude slower than just storing the handful of values directly. Checked for other reuse of Cook-Mertz's pieces outside tree evaluation too: `GF2n` currently has no callers besides `CookMertzField`, and a search for an existing need for Lagrange-interpolation-shaped tools elsewhere (Reed-Solomon-style erasure coding for corpus/state sync, secret-sharing-style fragment reconstruction) found none — the "corrupt"/"redundant" hits in `corpus_manager.py` are unrelated (bad tracker state, not data loss across processes), so wiring one in would be inventing a need rather than meeting one. Reconsider only if a future tree-evaluation component genuinely needs `h*l` bytes it cannot spare, or a genuine erasure-coding/secret-sharing need materializes — until then the module stays importable scaffolding, not a hotpath dependency.

### Scheduling Intelligence
- **Position schedulers and the position arena** (`core/schedulers/pos_base.py`, `pos_burn_front.py`, `services/position_arena.py`; `--position-arena`, `--burn-front`): *where* a mutation lands is a third scheduling axis beside operator and seed selection, with its own contract (`propose(data, buf_len) -> int | None`, `record(data, offsets, outcome, weight)`) and its own Elo tournament under `pos_<name>` keys (`Arena`/`strategy_arena()` in `core/analyzers/analyzer_elo.py` partition operator, `seed_` and `pos_` keys; stats, report and canary floors use it). Previously `OperatorEngine.select_position` called every enabled tracker (sensitivity, TE, phase, MI, crash-MI, region profile) and chose uniformly among the answers, so a tracker's quality never affected how often it was heard. With `--position-arena` and `--elo`, Thompson sampling over the `pos_` posteriors picks one arm per position; `uniform` is always pooled, first, and is the floor: `Fuzzer._check_canary_inspection` logs any proposer rated at or below `pos_uniform`. Only arms whose feature is on join the pool (no phantom opponents); an arm that declines gets a uniform offset but is charged under its own name (charging it to uniform made a decliner unbeatable: it only ever played as an opponent and won every miss round). Matches are per round: each arm that served a position plays each pool member that did not, with the round's surprisal-weighted score; arms that shared a round do not play each other. **`burn_front`** models a seed's byte offsets as steel wool: a coverage gain deposits Gaussian-kernel heat on the mutated offsets' bins (adjacent bytes are usually one field), each proposal burns 15% of its bin's fuel so a hot region exhausts and the front moves on, a gain refuels its bin, heat cools 10% every 32 proposals, and 10% of draws are uniform sparks. There is no time step -- conduction is one kernel deposit per gain, so nothing integrates or diverges. State is per parent seed, sparse (<=256 hot bins, <=4096 bins per seed) and LRU-bounded (256 seeds); persisted (`burn_front` state section, LRU order kept). It is credited off-policy on every round whoever served the positions; delocalised operators (`_DELOCALISED_OPS`) have no true offset and are never credited. `--position-arena` always fields burn-front as an arm; without the arena, `--burn-front` is one more candidate in the uniform pick. Both flags are off by default and enabled by `--hail-mary`; no paired run exists yet (plan: `tools/lib/bench_paired.py`, uniform baseline vs arena vs arena+burn-front, 12 seeds x 2k execs on fuzzgoat). The arena's pool costs one `select_strategy` call per position instead of computing every tracker's proposal. **`fibonacci`** (`core/schedulers/pos_fibonacci.py`, arena-only, implied by `--position-arena`): round-robin's binning, sized from the live buffer (not the parent seed, whose bins clamped onto the last byte of a shrunk buffer), visited in Fibonacci-hashing order, `bin_n = floor(frac(n/phi) * num_bins)` over one global counter. Any prefix spreads evenly (three-gap theorem), so the tail is hit on the second pick; no per-seed state, so unlike round-robin's 256-seed LRU it cannot be evicted back to bin 0 on large corpora. Signal-free; `record()` is a no-op.
...
- **Single-operand tracing (trace-div / trace-gep)** (`afl_shim.c` layer 3, `adapters/track_parser.py::pairs_from_operand_records`): clang's `-fsanitize-coverage=trace-div,trace-gep` hands over the runtime divisor of every non-constant division and the runtime index of every GEP — two input-derived values trace-cmp cannot see, since a division is not a comparison and an array index is only compared against its bound when the target checks it. The shim writes them as their own record kinds (`DIV <hex value> <width> 0x<pc>`, `GEP <hex index> <width> 0x<pc>`) rather than as `CMP` records with an invented opponent: `comparison_stats()`/`comparison_walls()` count CMP records to find unsatisfiable comparisons, and a divisor has nothing to be satisfied against. `collect_tokens()` folds each operand into the pair pool as `(observed, interesting)` — a divisor against 0 and 1 (division by zero, degenerate quotient), an index against 0 and the all-ones word (out of bounds both ways) — so the existing redqueen mutator consumes them with no new operator. Both sides drop operands below 256 (`CMPLOG_OPERAND_MIN` / `OPERAND_MIN_VALUE`): every loop counter is a GEP index and havoc reaches those values unaided. **Build note:** `trace-div`/`trace-gep` are modifiers, not coverage levels — clang emits no call sites unless a level (`trace-pc-guard`) is requested alongside, and at `-O1`+ an array index folded into an addressing mode is gone before the sancov pass runs, so GEP yield is optimization-level dependent. `build_targets.sh --vendor-tracecmp` carries the flags.
- **Validity channel (Zest)** (`core/validity.py`, `--reject-code N`): coverage alone cannot tell "reached new code" from "reached new code in the parser's error path". Zest (ISSTA'19) asks the harness: the target exits with `N` to report that it *rejected* the input, `0` to report acceptance, and anything else is UNKNOWN — a crash or a timeout is not a verdict on syntax. Coverage reached on accepted runs is banked in its own map (`ValidityChannel.valid_edges`), so an input that is valid and covers ground no valid input covered before is admitted even when the main map has seen those edges already: reached from the error path they lead nowhere, reached from an accepted input they are the semantic stages behind the syntax check. Edges from invalid and unknown runs are deliberately **not** banked — banking them would let an error-path execution claim coverage the first genuinely valid input to reach it should be credited with, and the channel would then answer the same question as the main map one execution later. `is_new_valid_coverage` joins both the `success` disjunction (bandit credit) and the admission disjunction; `seed_meta["valid"]` is recorded at admission and `SeedPicker._weight_validity` boosts accepted seeds by `VALID_SEED_BONUS` (1.5) — a boost, not a gate, since an invalid seed is often the shortest path to a branch inside the parser. `valid_rate` excludes UNKNOWN from its denominator, or it would track crash frequency rather than parser acceptance. Inert without `--reject-code`: every execution classifies UNKNOWN and no seed carries the metadata key.
- **SGFuzz enum-state transitions** (`core/state_vars.py`, `__sfuzz_state` in `afl_shim.c`): a parser keeps its state in an enum-typed variable and the assignments to it are the state machine's transitions — which edge coverage cannot see as transitions, because it records the *code* that performs one, so two runs visiting the same blocks in a different order collapse into one bitmap. `scan_enum_states()` finds `typedef enum {...} T;` and `enum T {...}` types, their constants, and the variables declared with each; `instrument_source()` rewrites `st = ST_BODY;` to `st = ST_BODY, __sfuzz_state(id, ST_BODY);` — a comma expression, so the assignment's value and side effects are unchanged and an unbraced `if` keeps working. Variable ids are a hash of the type name, not a scan counter: ids are baked into instrumented source, and discovery-order numbering would renumber every state when an unrelated enum is added above it. The runtime folds `(var_id, previous, current)` into the **edge map** via `__afl_map_edge` rather than into a channel of its own — SGFuzz keeps an explicit State Transition Tree, but an edge carries the same "this pair is new" signal and arrives already wired to scoring, scheduling, admission and the n-gram ring. Mixing the previous value in makes the *transition* the coverage item, not the state: reaching DONE from BODY and reaching DONE from INIT are different edges. **Two known limits**, both in `docs/TODO.md`: nothing can schedule *by state*, since the tree is not materialised; and declaration initialisers (`parse_state st = ST_INIT;`) are deliberately not instrumented — in a declaration the comma separates declarators, so the comma expression would parse as a second declarator and the source would not compile.
...
- **Discounted UCB** (`--ducb`, `--ducb-gamma`): Garivier & Moulines' D-UCB. Every per-arm count and reward is an exponentially discounted sum, so evidence from before a change point ages out of the mean *and* out of the confidence width — an abandoned operator's width re-widens and the arm is retried. This is what `arm_decay` on the Thompson and hierarchical posteriors reaches for and cannot express: that decay only shrinks the mean, and at 0.999 per 100 pulls removes ~6% of the mass over a 6000-pull campaign. Statistics are stored relative to one global discount factor (the trick `Exp3Scheduler` uses for its weights), so a record is O(1) and the O(K) renormalisation sweep only runs near float underflow. The paper's leading `2B` over-explores badly at this reward scale — at the paper's own coefficient the index sits barely above the uniform baseline — so `exploration` defaults to 0.25; the sweep is tabulated in the class docstring.
- **Sliding-window UCB** (`--swucb`, `--swucb-window`): the same paper's other policy. Only the last τ pulls count, maintained as a deque with incremental per-arm sums (O(1) amortised, O(τ) memory). Sharper than D-UCB on abrupt change points, noisier on stationary stretches, because its effective sample size is capped at τ however long the campaign runs. An arm whose last pull falls out of the window is indistinguishable from a fresh one, which is the mechanism that re-opens it. `xi` defaults to 0.15 rather than the paper's 0.6 for the same reason as above.
...
- **TPE** (`--op-tpe`, `core/schedulers/op_tpe.py`, BO-3): categorical Tree-structured Parzen Estimator. Splits the last 512 outcomes at the top-`gamma` (0.25) reward quantile, models `l(op) ∝ α_op + good(op)` and `g(op) ∝ β_op + bad(op)`, draws 24 candidates from `l` and keeps the largest `l/g` (hyperopt). Models density of good vs bad outcomes instead of a GP posterior. `supports_priors = True`: format priors seed `l`/`g`. Elo-only, unmeasured.
- **SLOPT** (`--slopt`): Koike et al.'s bandit-friendly mutation scheme (ACSAC 2022). Each round selects one operator (with whichever operator scheduler is active) and applies it 2^t times; the batch exponent t ∈ 1..7 is drawn by Thompson sampling from a bandit keyed by (seed-size group, operator), groups [0,10²), [10²,10³), [10³,10⁴), [10⁴,10⁵), [10⁵,∞), and the drawn arm is credited with that operator's round outcome. The learned batch replaces `-M`, perf-score scaling and the stall-recovery floor, since the arm must be credited for the batch actually applied. `core/slopt.py`.
- **Combinatorial UCB** (`--cucb`, `--cucb-gamma`): Chen et al.'s CUCB, the only scheduler here that treats the round's operator stack as a **superarm** rather than N independent pulls. `_record_outcome` runs `mutations_per_input` operators against one input, gets one binary outcome, and hands that same outcome to every operator in the round; measured on the convergence harness, that bundling shrinks the posterior separation between a p=0.30 and a p=0.05 operator from 0.250 at a stack of 1 to **0.029 at the shipped default of 8** — an 8.6× loss of the signal every operator scheduler is trying to read. CUCB recovers the per-arm rate by **inclusion contrast**: the mean reward of rounds containing an arm, contrasted against the mean of rounds without it, inverted through the noisy-OR round model. Where the out-sample is too small the contrast falls back to the global mean, which is biased toward zero but stays on the same scale — mixing scales was the defect that made an incumbent arm unbeatable in the first cut. `contrast_coverage()` reports how often the identifying branch was used (0.92–0.99 on the harness). Superarms are *not* de-duplicated: forcing distinct stacks costs about a third of the round yield without improving coverage, since on independent arms the optimal stack is the best operator repeated.
- **Softmax operator scheduler** (`--softmax`, `--softmax-tau`): ranks arms by softmax of their empirical mean reward. Temperature `tau` controls exploration — low values concentrate on the current best arm, high values approach uniform selection. `record()` uses `reward = weight if success else 0.0`, matching the project-wide convention so failed observations contribute zero reward while failures still count in the arm's sample count. Selection uses max-shifted exponentials `exp((mean - max_mean) / tau)` for numerical stability and draws once against normalized probability mass. Equal means fall back to the first candidate in `ops`, so ties are deterministic and reproducible.
- **Top-K operator scheduler** (`--topk`, `--topk-k`): keeps a running weighted mean for each arm and selects uniformly among the top-k arms by that mean. `k` controls the exploration/exploitation trade-off directly — `k=1` degenerates to greedy mean selection, larger values preserve uniform exploration among the leading arms. `record()` uses `reward = weight if success else 0.0`; invalid `k <= 0` is rejected at construction time.

  All three take the shared `RandPool` (Hard Rule 16) so `--seed` reproduces the campaign. `tests/test_scheduler_convergence.py` asserts that all three recover after the best arm's yield collapses — the regime `MonteCarloScheduler` and `EpsilonGreedyScheduler` are pinned as failing — and asserts their stationary tail share separately, because a sliding window and a per-round discount both pay a permanent exploration tax that makes cumulative regret linear by design.
...
- **Record-phase extrapolation of the TE map** (`core/circular_stats.py`, automatic under `--transfer-entropy`): `update_te_causal_map` only observes absolute offsets below its own 64-byte cap, so `get_te_weighted_position` can never name a position outside it, and a seed's inferred `record_stride` was until now used only to shuffle whole records and to gate field hypotheses onto record boundaries. Folding each causal offset to a phase on the record (`theta = 2*pi * (o mod L) / L`) turns "which field inside the record matters" into a circular question: the Kuramoto order parameter `r = |sum(w_k exp(i theta_k))| / sum(w_k)` measures how phase-locked the offsets are and `psi` names the field. The same `r` is the Rayleigh statistic under the uniform-phase null, so the lock is accepted at a stated alpha (`DEFAULT_ALPHA = 0.01`) via Wilkie's closed form rather than a tuned ratio, and a confirmed lock extrapolates the cheap local evidence to `offset + k*stride` across the whole buffer — which is the inference the linear position maps structurally cannot make. A rejected lock returns `None`, which is the more important half. **Significance uses Kish's effective sample size** `(sum w)^2 / sum(w^2)`, not the observation count: edge-count weights are wildly unequal, and a single dominant position is phase-locked with itself by construction, so raw `n` would manufacture a result (pinned by `test_one_dominant_weight_is_not_significant`). The p-value is calibrated against a 20k-trial Monte-Carlo null in `tests/test_circular_stats.py`. Hot-path shape matters here: the concentration test is 19.5-29.6 us per call and the position is drawn every mutation, so `phase_lock` (expensive, changes only on the TE observation cadence) is split from `draw_phase_position` (cheap) and memoized on `Fuzzer._te_causal_version` — 0.43 us per mutation, 50x. The result joins `sens`/`te`/`mi`/`crash_mi`/`region` as one more uniformly drawn candidate in `select_position`; it is never the sole source.
...
- **Learned chunk order for ten container formats** (`wfc_reorder_learned`, `core/wfc_chunks.py`): the PNG/JPEG/BMP reorderers above use hand-written constraint sets; this operator learns one `AdjacencyTable` per format from the kind sequence of every corpus seed admitted with new coverage, then collapses a 1-D grid over it. Formats: isobmff, riff, webp, gif, ogg, flv, nal, asf, mpegts, zip (webm is the one with a parse/serialize pair still unwired). A tile is the chunk's role, chosen so that two chunks of one tile are interchangeable: box fourcc, RIFF fourcc, GIF node kind, Ogg (BOS/EOS role, stream serial) so multiplexed-stream interleaving is visible, FLV tag type, NAL unit type, ASF object GUID, TS PID, and for ZIP the well-known ordering-sensitive names (`mimetype`, `[Content_Types].xml`, `META-INF/*`) as their own tiles, else the extension (`serialize_zip` recomputes offsets and counts, so a reorder never desynchronises the central directory). `strict` mode collapses under the table as observed; `violate` mode adds exactly one unobserved pair and pins two adjacent cells to it, an ordering one step outside anything the corpus has shown. The first chunk (GIF header, Ogg BOS, ASF header object) and last (GIF trailer, Ogg EOS) stay pinned where the format needs them. Chunks a collapse over- or under-places are kept, placed at a table-legal slot, and a collapse that still emits an unobserved adjacency is retried up to 8 times. Declines (returns `None`) when the input has fewer than three chunks or the reorder equals the input. Tables learn only under `--wfc` (admission calls `REGISTRY.notify_new_coverage`) and persist in the `wfc_tables` state section; a fresh run starts empty
...
- **`--report` flag**: full explainability report with coverage, mutations, perplexity, corpus health, edge map. The Run Summary opens with the **exec line** (target invocation reconstructed from `target` + `target_args` + `file_mode`, rendering `{file}` as `@@` AFL-style) plus the full **invocation** (`sys.argv` captured in `cmd_fuzz`) and input mode. A **Configuration** section (seed, schedule, mutations/input, resume, max len, timeout, map size, multi-target count, in-process/direct-lite/persistent, extra crash codes, ASAN/UBSAN targets, cmplog, dictionary, grammar, markov, operator count) appears after the Run Summary, and a **Crash Signatures** histogram (from `crash_sigs`, keyed by `SanitizerReport.signature` or `signal:N@0xaddr` fallback, top 2 frames appended when tracked) follows Crash Analysis. When more than one signature is present, signatures are additionally grouped into **stack-similarity clusters** (`core/crash_metadata.py::cluster_crashes`, order-aware Levenshtein over normalized frame sequences with a union-find merge) so crashes that differ only by inlined frames or instruction offsets report as one likely bug instead of inflating the signature count. Because single-linkage can chain two genuinely distinct bugs together through an intermediate signature, each multi-member cluster is also checked by `core/crash_metadata.py::detect_chained_clusters` (a read-only diagnostic, not a second clustering pass) for its worst-case pairwise similarity; a cluster below the core threshold (`FUZZER_CRASH_CLUSTER_CORE_THRESHOLD`, default 0.5) is annotated `[CHAINED -- ...]` in the report so triage doesn't trust its grouping blindly.
...
### Shim Edge Analysis (`tools/edge_diagnostic.py`)
- **Consolidated diagnostics**: `matrix`, `phantom`, and `stored-ids` sub-commands gather the former `edge_matrix_analysis.py` and `phantom_edge_probe.py` into one tool. `matrix` replays a corpus against an instrumented target, captures the AFL SHM edge bitmap (positions/ids/counts) per input, and reports the marginal distributions, seed×edge spectrum, GF(2) rank/minimal-basis, and (with `--positions`) the placement matrix. `phantom` runs the same collection under input/seed shuffle to measure id instability (ASLR-context phantom edges) and which seeds are affected.
- **`matrix --hail-mary` growth pass** (`--hm-target`, `--iters`): before collecting, the tool runs a real `fuzzer-tool fuzz --hail-mary` campaign against a *copy* of the corpus (the original is never mutated) and analyzes the grown corpus, so the shim statistics reflect edges the fuzzer actually discovered rather than only the seed set. `--iters` is a real execution budget (`--max-execs`), not `-n` iterations — one iteration runs a whole mutation budget, so `-n 2500` silently turns into ~400k execs; the budget keeps the campaign honest. Because `--hail-mary` force-enables in-process mode, the campaign target must be a dlopen-able `.so`; the tool auto-derives one from the PIE target's sibling (`_noasan.so` preferred — an `_asan.so` can double-load libasan and trip an ASAN CHECK in-process), overridable with `--hm-target`. The `.so`'s exposed `fuzz_shm_run` symbol is used via `--hm-inprocess-func`. StateStore sidecar files (`state.json`, `edge_tracker.json`, `markov.json`, ...) the campaign drops into the corpus dir are stripped before collection so they are never executed as fuzz inputs.
- **`matrix` sections [11]-[18]** (`tools/edge_matrix_modes.py`): continue the section numbering past `--flow` (P1-2, closed negative, F17). `--flaky K` (section [11]) reruns every input K times live and reports edges whose presence or count varies across repeats — a false-novelty source, since a flaky edge can make a seed look like it privately owns coverage it does not reproducibly reach. The rest run offline on a saved (`--save`/`--load`) or freshly collected seed×edge matrix, individually or all via `--all-offline`: `--subsumption` [12] (seed-side subset poset — subsumed/maximal seeds, antichain width, longest chain — and edge-side implications GF(2) elimination cannot see); `--admission-replay` [13] (replay the corpus through new-edge / new-(edge,AFL-count-class) / new-max-count admission, corpus order vs `--resamples` shuffles, plus a binary/bucket/log1p/raw resolution ladder); `--rarefaction` [14] (accumulation curve vs its permutation null, and Chao2 richness fitted on a fraction of the corpus checked against the full union — calibrates `EdgeTracker.good_turing_estimate`); `--bootstrap N` [15] (seed-resampling CIs on the headline GF(2)/rank/cover statistics); `--length-confound` [16] (is hit volume just input size — needs sizes, carried by `--save`/`--load` since this pass); `--prefix-fold` [17] (distinct rows and rank as the `edge_id`'s low context bits are folded away — bit-prefix bucketing, the sanctioned positional statistic per `handover_edge_id_axis_2026-09-18.md`); `--score-audit` [18] (screens candidate seed scores — mass, the `seed_residual` score, PC1, rank-k leverage/subspace distance — against leave-one-out edge/class loss, partialling out total hits and degree; a screening filter only, matching Tang's F-14 finding: it can retire a score, never promote one, which still needs `bench_paired.py`).
...
# targets are consolidated into fgrep_read input modes:
#   mode 0x04 = pattern matching, mode 0x05 = regex compilation,
#   mode 0x06 = full search pipeline (first byte of the input selects)

# Multi-target: fuzz multiple binaries with shared corpus (glob supported)
fuzzer-tool fuzz targets/png_read.so targets/zlib_read.so -d corpus/
...
fuzzer-tool fuzz 'targets/*_read.so' -d corpus/

# Two-pass workflow: fast fuzz without ASAN, then verify crashes with ASAN
fuzzer-tool fuzz targets/png_read_nosan -d corpus/fast/
fuzzer-tool verify targets/png_read_asan.so corpus/fast/crashes/
...
| `--splice-donor uniform\|gravity` | Splice-family donor pick: uniform, or gravity model over MinHash edge complements with PPML-fitted exponents |
...
| `--bo-gp-ucb` | Bayesian Optimization GP-UCB with Expected Improvement acquisition |
| `--bo-gp-length-scale FLOAT` | BO-GP RBF kernel length scale (default: 1.0) |
| `--bo-gp-noise FLOAT` | BO-GP observation noise sigma (default: 0.01) |
| `--ducb` | Discounted-UCB operator scheduling (Garivier & Moulines) |
| `--ducb-gamma FLOAT` | D-UCB discount per pull; memory is 1/(1-gamma) pulls (default: 0.9999) |
| `--swucb` | Sliding-window UCB operator scheduling |
| `--swucb-window INT` | SW-UCB window length in pulls (default: 4000) |
| `--cucb` | Combinatorial UCB: scores the round's operator stack as one superarm |
| `--cucb-gamma FLOAT` | CUCB discount per mutation round (default: 0.9995) |
| `--kl-ducb` | KL-UCB variant of D-UCB: uses Bernoulli KL upper bound for confidence width |
| `--kl-ducb-gamma FLOAT` | KL-D-UCB discount per pull (default: 0.9999) |
| `--kl-swucb` | KL-UCB variant of SW-UCB: uses Bernoulli KL upper bound for confidence width |
| `--kl-swucb-window INT` | KL-SW-UCB window length in pulls (default: 4000) |
| `--slopt` | SLOPT: one operator per round applied 2^t times, t learned per seed-size group and operator |
| `--moss` | MOSS operator scheduling: UCB with no exploration bonus past an operator's fair share |
| `--moss-gamma FLOAT` | MOSS discount per pull; 1.0 never forgets (default: 1.0) |
| `--bayes-ucb` | Bayes-UCB operator scheduling: Beta-posterior quantile index, uses format priors |
...
Import seeds (and crashes, for AFL) from another fuzzer's output directory, auto-detecting AFL/libFuzzer/honggfuzz layout when `--format` is omitted.

```bash
fuzzer-tool import <source_dir> -d <corpus> [-o <crashes>] [--format afl|libfuzzer|honggfuzz] [--autotokens FILE]
```

| Flag | Description |
|------|-------------|
| `-d DIR` | Destination corpus directory |
| `-o DIR` | Destination crashes directory (AFL only) |
| `--format` | Source format (default: auto-detect) |
| `--autotokens FILE` | Also tokenize the destination corpus into a whole-token AFL-format dictionary written to `FILE` |
...
`tools/benchmark.py` is the single entry point; harnesses live in
`tools/lib/`. `benchmark.py list` names every harness; `benchmark.py <name> [args...]` execs it with args verbatim
...
`huffman`. A new `tools/lib/bench*` script must be added to its `_REGISTRY`;
...
tools/benchmark.py smoke targets/png_read 10000
```
...
of these four named configurations), use `tools/lib/bench_sweep.sh`. Both scripts
