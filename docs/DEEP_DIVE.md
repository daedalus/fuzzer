...
### Corpus Management
- **Desynchronized corpus sync** (`core/desync.py`, always on under `-j N`): `_sync_corpus_in` ran on a fixed wall-clock period, identically in every worker, with nothing coupling them — and each sync lists every sibling's corpus recursively, measured at **0.25 s per worker for 8 workers holding 4000 seeds each** on a warm cache, growing with the corpus. Uncoupled oscillators at one frequency random-walk against each other and spend as much time near-collision as anywhere, so the fleet periodically stalls together and hits the filesystem in the same instant. The coupling wanted here is the *repulsive* one: DESYNC (Degesys et al., IPSN'07) moves each node a damped fraction toward the midpoint of its two phase neighbours on firing, which for N nodes means 1/N spacing. Workers publish their firing time to a dotted `.sync_phase` file in their own corpus dir — outside `seeds/`, the only subtree sync reads — and read their siblings'; files staler than 4 periods are ignored (a dead worker would otherwise pin a slot and the survivors would spread around a ghost), as are future timestamps and partial reads. **This spreads the cost, it does not reduce it**; the O(workers x corpus) rescan is a separate problem. `DEFAULT_DAMPING = 0.5` is measured, not inherited from the paper: over 12 seeds x n in {4, 8, 16}, 600 firings with 1.5 s of per-round drift, median final coherence by damping is 0.148 (0.2) / 0.129 (0.3) / **0.113 (0.5)** / 0.117 (0.8) / 0.152 (1.0) / 0.210 (1.2) against 0.250 uncoupled — and the tails are the point, since uncoupled p90 is 0.848 with a worst case of 0.973 (a fully coherent fleet) while 0.5 caps p90 at 0.204 and the worst case at 0.237. **Known limitation, pinned as a test:** the midpoint rule has no gradient inside a tight cluster, so a cold cluster peels only from its edges and the peeled nodes leapfrog as a group — six workers started inside a 1 s window sit at coherence 0.62 after 600 firings and no better at 2000. That is why `initial_offset` stakes out the spacing at startup from the worker index (an asymmetry a physical oscillator ensemble does not have) instead of trusting convergence; the coupling only has to hold it afterwards.
