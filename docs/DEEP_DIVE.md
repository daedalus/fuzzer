...
- **Katz ICFG footprint**: `KatzChannel.build` releases the per-function CFGs after the probe table and `upload()` drops `TargetDistance` (it holds the whole binary); `node_addrs` is a sorted `array('Q')` looked up by bisect. ffmpeg (2.74M nodes): 2112 -> 217 MB retained.
