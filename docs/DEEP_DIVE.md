...
- **PLL monitor** (`--pll`, `core/analyzers/analyzer_pll.py`, 2026-09-24): observation layer over `core/pll.py`. Exec times are pushed per execution (one array append), discovery-rate deltas at each stats tick; each series buffers 256 samples, bootstraps a `PhaseLockedLoop` from `detect_periodicity` (retries on a miss), and logs lock/unlock transitions with the stall-recovery flag. `stall_lift = P(stall | transition) / P(stall)`. Consumers: stats field `pll: t:<period><L|u> d:…` and a "PLL" line per series under Spectral Diagnostics (tracked period next to the batch FFT). Read-only. fuzzgoat: exec-time series bootstraps at period ≈ 5, stays unlocked.
