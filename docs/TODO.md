...
- [x] **A/B the cost-based Boltzmann energy** (2026-08-29, run 2026-08-30) — **no significant difference, bounded.** png: cost wins 3 / loses 6 / ties 1, median -2.0 edges, McNemar p = 0.508. jpeg: 2 / 5 / 3, median -1.5, p = 0.453. Ten seeds, three replicates per cell, `direct_lite` at `-m 65536`. The design had ~100% power at a 10-edge effect and ~77% at 5, so the usable statement is that the change moves edge discovery by under ~5 edges either way, not that it does nothing. Direction leaned consistently against the change on both targets (11 losses to 5 wins pooled), which is the risk the item named, but it is not significant. Full write-up and the two harness changes it forced in `docs/learnings/2026-08-30-boltzmann-ab-result.md`. **`grep_read.so` was not run** — best remaining carrier of the cost signal since DFA construction makes its per-seed cost vary most, but ~250 s/cell puts a paired 10x3 run at ~4 hours.
