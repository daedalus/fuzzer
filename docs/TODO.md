...
- [ ] **ffmpeg RSS: Katz horizon recompute peaks ~3.9 GB** (2026-09-24) — `ffmpeg_read_asan.so`, 480 seeds, `--wfc`, in-process ASAN. Startup is fixed (WFC line 2718 -> 830 MB, after calibration 3183 -> ~1107 MB): the ICFG no longer keeps CFGs/`_td`/int lists, blocks are slotted, assembly is packed arrays, ASAN releases to the OS. Remaining peak is the first `KatzChannel.ensure_scores()` in the fuzz loop: `core/horizon.py::build_horizon_graph` builds Python adjacency lists, edge-tuple sets, Tarjan lists and a 2.7M-entry `node_index` dict over all nodes; `katz_scores` and the gzip state save each reach ~2.2-2.6 GB. Candidate: CSR `np.ndarray` adjacency from the already (u, v)-sorted `src`/`dst`. No CLI switch disables the channel. Also open: per-seed edge state (`seed_edges` sets + `seed_hit_counts` dicts, ~0.35 MB/seed at ~4k edges; one int object per seed per edge, 4M for ~44k unique ids).
