...
- [ ] **`EdgeTracker._maybe_prune` has not run since `fe8fd42`** (2026-08-29) — `max_tracked_seeds` went from 200 to 200,000 in that commit ("perf: skip Katz ICFG when no targets and reduce seed-picker/calibration overhead", 2026-08-27), a 1000x change the message does not mention. Verified: 500 seeds recorded, zero pruned; campaigns in the cost-ledger measurement reached 221-403 seeds. Both consequences want deciding, and neither is obviously wrong — only undecided. (1) The memory bound is gone: `seed_edges`, `seed_hit_counts`, `seed_edge_traces` and six companion maps now grow with the corpus, and on a target like `ffmpeg_read` with ~8,189 live edges that is not free. (2) The `_edge_owner_count` rebuild lives inside `_maybe_prune` and therefore no longer runs; it was added because stale owner counts inflate `mean_owners`, suppress the crowding bonus and push edges past `RARE_EDGE_OWNERS` — i.e. it degrades the rarity signal that drives the schedule. If the raise was deliberate, the rebuild needs a new home and the two stale `(200)` comments (now corrected) were the only trace of the old contract. This also blocks the third consumer in backlog D5, the cost-aware age fallback: a cost term inside a function that never executes is dead code.
