...
- [ ] **ffmpeg startup RSS: ~3.1 GB before the first mutation** (2026-09-24) — measured on `ffmpeg_read_asan.so`, 480 seeds, `--wfc`. (1) Katz ICFG build (`services/katz_channel.py`, 2.7M nodes) peaks 3.4 GB and leaves ~2.2 GB. (2) Per-seed edge state: `seed_edges` sets + `seed_hit_counts` dicts, ~0.35 MB/seed traced at ~4k edges (ffmpeg seeds reach 10k) — a sorted `array('I')` is 4 B/edge against ~75 B for set slot + int object, but every reader expects `set`/`dict`. (3) Preloaded ASAN serves Python's mallocs too: traced 425 MB became 1.1 GB RSS, and freed Python tables fill the 256 MB quarantine. The target itself does not leak (flat 59 MB over 1440 execs, `quarantine_size_mb=0`).
