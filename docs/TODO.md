...
- [x] **Seed baseline calibration** (FIXED 2026-08-24) — root cause of the "coverage plateaus instantly / sat:100% on tiny universes" symptom class: `fuzz_one()` executes ONLY `_dedup_mutate()` output, so corpus seeds' pristine bytes were never run and structured-format campaigns started from wrapper-only noise (png_read.so: 11 valid PNGs → shm=6 after 150 execs; one direct seed execution = ~330 edges). Fix: `Fuzzer._calibrate_seed_baselines()` runs each loaded seed verbatim once at startup through the normal coverage pipeline; crash/timeout seeds counted, not fatal; single-target SHM mode only. Measured: 96 PNGs → 893 baseline edges in 0.7s. Tests: `tests/test_regression_seed_baseline.py` (falsification: baselines enter tracker + GT sees real universe; adversarial: crashing seed survives startup). Diagnostic note: `sat:` is Good-Turing `1 - undisc/total` with N2<10 damping — it pins to 100% whenever the universe estimate collapses, which makes a starved tracker LOOK saturated.
...
- [x] **FFmpeg container mutators: Ogg/FLV/ASF/RIFF wired in** (IMPLEMENTED 2026-08-25) — `mutations/{ogg,flv,asf,riff}.py` existed from a prior session but were never registered: no `_FORMAT_SNIFFERS` entry, no `_CATEGORIES["format"]` membership, no `_op_*_chunk_mutate` handler on `OperatorEngine`, so all four were dead code reachable by nothing. Wired following the `mpegts_chunk_mutate`/`nal_chunk_mutate` pattern exactly (sniff-then-mutate-or-generate in the handler). Also fixed a real bug found while wiring: `ogg.py` and `asf.py` called `rng.getrandbits(...)`, a stdlib `random.Random` method the fuzzer's rng contract doesn't guarantee — `ExhaustivePool` (the enumeration-mode rng) has no such method and raised `AttributeError` the first time either mutator's random-generation path was exercised outside a live run. Replaced with `rng.randint(0, N)`, matching the convention every other format mutator (`isobmff.py`, `nal.py`) already follows. Tests: `test_ogg_mutator.py`, `test_flv_mutator.py`, `test_asf_mutator.py`, `test_riff_mutator.py`.
- [x] **ADTS AAC and MP3 raw elementary-stream mutators** (IMPLEMENTED 2026-08-25) — `nal_chunk_mutate` only covered H.264/H.265 Annex-B; raw audio elementary streams (`.aac` ADTS, `.mp3`) had no operator at all. New `mutations/adts.py` and `mutations/mp3.py`, each scanning for the format's own syncword to find frame boundaries (12-bit 0xFFF for ADTS, 11-bit for MP3) rather than trusting a computed frame-length field, the same choice `nal.py` makes for start-code scanning over its own `nal_unit_type`-derived lengths. Sniffers (`adts_chunk_mutate`, `mp3_chunk_mutate`) are cross-checked against each other's near-identical all-1s sync prefix so neither false-triggers on the other's stream: ADTS requires its layer-equivalent field fixed at 0, which MP3's sniffer explicitly treats as the reserved (rejected) layer value. Tests: `test_adts_mutator.py`, `test_mp3_mutator.py`.
...
- [x] **Port Tier-1 candidates from the 2026-08 web survey — trivial items** (IMPLEMENTED 2026-08-24) — 3 of 7 low-effort items landed: **autotokens** (`services/import_corpus.py::build_autotoken_dictionary`/`extract_tokens`, `import --autotokens FILE`), **entropic power schedule** (`core/schedules.py::SeedScorer._entropic_factor`, `--schedule entropic`), and **stack-hash crash clustering** — `core/crash_metadata.py::cluster_crashes` already existed but was unwired; now called from `services/report.py::_crash_signatures` to group signatures into stack-similarity clusters. The remaining 4 items (trace-div/gep, n-gram edges, Zest validity channel, SGFuzz state variables) are unstarted: trace-div/gep and n-gram edges require `afl_shim.c` changes, Zest/SGFuzz are `L` effort requiring new feedback plumbing. Survey: `docs/web_research_port_candidates_2026-08.md`.
