...
- [ ] **`read_bitmap()` / `runner.py` disagree with the SHM layout, inertly** (2026-08-23) — `read_bitmap()` reads `shm_size` BYTES from the segment base, but `shm_size` is the entry count and the edge table starts at `SHM_METADATA_SIZE`. `runner.py:167` then compares `len(bitmap) <= shm.size` (bytes against entries) and memmoves to `shm._ptr`, not `shm._ptr + SHM_METADATA_SIZE`. All of it is currently harmless for one reason only: `cov_env_id = self.shm_cov.env_id`, so the runner attaches to the SAME segment the fuzzer owns and the memmove is a self-copy. It becomes a misaligned 24-byte shift the moment those two segments differ. Fix the offset and the guard together, or not at all — half of this change is worse than none.
