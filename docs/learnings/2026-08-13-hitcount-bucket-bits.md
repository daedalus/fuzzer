...
Direct indexing is affordable because edge ids are bounded. When this was written that bound came from `trace_pc_guard_init` handing out small sequential guard values, which kept `edge_id = prev_loc ^ cur_loc` below roughly `2 * guard_count`. **Correction (2026-09-23): that small range was not a property to lean on, it was a defect.** Sequential guards plus `edge_id |= 1` made the id function merge distinct edges -- 344 real edges on fuzzgoat reached Python as 145 ids (handover_edge_id_axis F16). Since the hashed-location shim change (`fix(shim): stop the edge-id function from merging distinct edges`), guard and hand-written locations are hashed into `ceil(log2 N) + __AFL_GUARD_SLACK_BITS` bits, capped at `__AFL_GUARD_MAX_BITS = 24` precisely so every id stays below `VIRGIN_DENSE_MAX` and this dense array keeps its fast path; the cost is that the array now grows toward `2^bits` instead of `2N` (about 6.5 MiB at fuzzgoat's 2^19, up to ~200 MiB at the cap across the per-id arrays). `__AFL_CTX_BITS` in the 24..32 range still scatters ids across the whole u32 space; those go to a dict (`VIRGIN_DENSE_MAX`).
