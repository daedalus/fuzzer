...
4. **The from-array win only applies when the buffer already exists.** Attempted the same trick on seed_picker._compute_weights (4x `np.array(list)` → `array('d')` + frombuffer): interleaved A/B at 1000 seeds measured ~4% SLOWER (1269→1319µs/call median). The data must be gathered element-by-element either way, and `array.append` (~46ns) costs more than `list.append` (~26ns), which outweighs the ~36ns/element conversion saving. The 44x "array.array→ndarray" number only holds when the buffer is already populated (socket/mmap/struct input) — not for a list you have to build first. When an audit candidate is "convert the list you just built", benchmark the gather loop too: the append path IS the cost. (Reverted; kept an implementation-agnostic weights contract test.)
