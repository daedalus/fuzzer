...
int fuzz_test(const unsigned char *buf, size_t len) {
    if (len >= 6 && memcmp(buf, "CRASH", 5) == 0) {
        if (buf[5] == 'S') {
            /* NULL-jump via a volatile pointer: at -O2 clang would otherwise
             * eliminate a direct NULL call as UB (fuzz_test compiled to just
             * `ret`, silently removing the target's crash path). */
            void (*volatile crash_fn)(void) = (void (*)(void))0;
            crash_fn();
        }
        if (buf[5] == 'A') {
            abort();
