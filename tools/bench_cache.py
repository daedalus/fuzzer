...
cache_path = os.path.join(os.path.dirname(TARGET), os.path.basename(TARGET) + ".profile_cache")
...
def clean():
    ...
    subprocess.run(
        [
            "sh",
            "-c",
            'for i in /dev/shm/*; do [ -f "$i" ] && echo "$i" | grep -q fuzzer && rm -f "$i"; done 2>/dev/null',
        ],
        capture_output=True,
    )
...
print(
    f" Corpus: {CORPUS} ({len(os.listdir(os.path.join(CORPUS, 'seeds')))} seeds)"
    if os.path.isdir(os.path.join(CORPUS, "seeds"))
    else ""
)
...
print(
    f"  {'Cold (no cache)':<20s} {m_cold:>8.2f}s {mn_cold:>8.2f}s {mx_cold:>8.2f}s {len(times_cold):>6d}"
)
print(
    f"  {'Warm (cache hit)':<20s} {m_warm:>8.2f}s {mn_warm:>8.2f}s {mx_warm:>8.2f}s {len(times_warm):>6d}"
)
print(
    f"  {'Cached + resume':<20s} {m_res:>8.2f}s {mn_res:>8.2f}s {mx_res:>8.2f}s {len(times_resume):>6d}"
)
