...
class Grammar:
    ...
    def _mutate_literal(self, data: bytes, max_len: int) -> bytes:
        ...
        buf = bytearray(data)
        idx = (self._rng or random).randint(0, len(buf) - 1)
        op = (self._rng or random).randint(0, 3)
        if op == 0:
            buf[idx] ^= 1 << (self._rng or random).randint(0, 7)
        elif op == 1:
            buf[idx] = (self._rng or random).randint(0, 255)
        elif op == 2:
            buf[idx] = (self._rng or random).choice([0, 1, 0x7F, 0x80, 0xFF])
        else:
            # Replace with a printable ASCII character
            buf[idx] = (self._rng or random).randint(0x20, 0x7E)
    ...
    def _mutate_truncate(self, data: bytes) -> bytes:
        ...
        if len(data) <= 1:
            return data
        cut = (self._rng or random).randint(1, len(data) - 1)
        return data[:cut]
    ...
    def _mutate_extend(self, data: bytes, max_len: int) -> bytes:
        ...
        pos = (self._rng or random).randint(0, len(data))
    ...
    def _mutate_replace_section(self, data: bytes, max_len: int) -> bytes:
        ...
        if len(data) <= 1:
            return self.generate(max_len=max_len)
        start = (self._rng or random).randint(0, len(data) - 1)
        end = (self._rng or random).randint(start + 1, min(start + 32, len(data)))
...
class TreeMutator:
    ...
    def _tree_swap(self, tree: TreeNode, max_len: int) -> bytes:
        ...
        target = (self._rng or random).choice(targets)
    ...
    def _tree_duplicate(self, tree: TreeNode, max_len: int) -> bytes:
        ...
        if not parents_with_children:
            return tree.serialize()[:max_len]
        parent = (self._rng or random).choice(parents_with_children)
        idx = (self._rng or random).randint(0, len(parent.children) - 1)
    ...
    def _tree_rule_sub(self, tree: TreeNode, max_len: int) -> bytes:
        ...
        if not multi_targets:
            return tree.serialize()[:max_len]
        target = (self._rng or random).choice(multi_targets)
        replacement_bytes = self.grammar.generate(target.rule, max_len=max_len)
