...
class TestStructureFunctionDetector:
    def test_init(self):
        d = StructureFunctionDetector()
        assert d.n_samples == 0
        assert not d.buffer_full
        assert d.noise_type() == "unknown"
        assert d.noise_slope() is None

    def test_sdev_constant_series(self):
        """structure-function deviation of a constant series should be ~0."""
    ...
    def test_sdev_requires_samples(self):
        """sdev(tau) returns NaN when n < tau+1."""
        d = StructureFunctionDetector(max_buffer_pow=4, min_samples=4)
        d.update(1.0)
        d.update(1.0)
        assert math.isnan(d.sdev(2))  # n=2, tau=2 → need 3
        d.update(1.0)
        assert math.isfinite(d.sdev(2))  # n=3, tau=2 → ok

    def test_sdev_power_of_two(self):
        """sdev works for various tau values."""
    ...
    def test_noise_type_insufficient_data(self):
        ...
        d = StructureFunctionDetector(max_buffer_pow=4, min_samples=20)
    ...
    def test_buffer_full(self):
        ...
        d = StructureFunctionDetector(max_buffer_pow=3, min_samples=2)
        assert not d.buffer_full
        for _ in range(8):
            d.update(1.0)
    ...
    def test_save_load_roundtrip(self):
        ...
        d1 = StructureFunctionDetector(max_buffer_pow=6, min_samples=4)
        for v in _white_noise(32, scale=2.0):
            d1.update(v)
        saved = d1.save()

        d2 = StructureFunctionDetector()
        d2.load(saved)
...
class TestStructureFunctionDispersion:
    """Tests for StructureFunctionDetector.dispersion()."""
...
class TestDispersionSignificance:
    ...
    def test_structure_function_detector_matches_dispersion_index(self):
        """StructureFunctionDetector's significance methods delegate to
        ...
        avd = StructureFunctionDetector(max_buffer_pow=10, min_samples=2)
        di = DispersionIndex(window=300)
        for v in values:
            avd.update(v)
            di.update(v)
