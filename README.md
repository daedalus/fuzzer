...
**Known limitation**: ASAN-detected memory bugs (heap/stack buffer overflow, use-after-free) do NOT trigger in direct_lite mode due to interactions between mid-process ctypes loading, PLT resolution ordering, and ASAN's initialization sequence. Standalone executables and subprocess-based modes (fork + `LD_PRELOAD`) detect ASAN bugs correctly. Direct_lite mode for ASAN `.so` provides throughput (~124k eps for `png_read_asan.so`) without crash isolation — crashes may abort the fuzzer process.
